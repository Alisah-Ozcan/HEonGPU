...
int main(int argc, char* argv[])
{
    ...
    heongpu::Ciphertext<heongpu::Scheme::CKKS> cipher_boot =
        operators.slim_bootstrapping_v2(C1, galois_key, relin_key,
                                        swk_dense_to_sparse,
                                        swk_sparse_to_dense);
    ...
    std::vector<Complex64> message_complex;
    message_complex.reserve(slot_count);
    for (int i = 0; i < slot_count; i++)
    {
        message_complex.push_back(Complex64(message[i], 0.0));
    }

    heongpu::PrecisionStats prec_stats =
        heongpu::get_precision_stats(message_complex, decrypted_1);
