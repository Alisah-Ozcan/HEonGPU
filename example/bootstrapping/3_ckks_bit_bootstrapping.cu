...
int main(int argc, char* argv[])
{
    ...
    // The scale is set to 2^41, resulting in 41 bits of precision before the
    // decimal point.
    double scale = pow(2.0, 41);
    ...
    std::vector<double> message(slot_count, 1);
    message[0] = 0;
    message[1] = 0;
    ...
    operators.generate_bootstrapping_params(
        scale, boot_config,
        heongpu::logic_bootstrapping_type::BIT_BOOTSTRAPPING);
