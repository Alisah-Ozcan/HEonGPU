...
#pragma unroll
                                             ...
                                             int current_decomp_mod_count)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < current_rns_mod_count; i++)
            {
                ...
                out1[index + (i << n_power) + location] = reduced_result;
            }
        }
    }
                                                  ...
                                                  int decomp_mod_count)
    {
        ...
        if (block_z == 0)
        {
            output0[index + (block_y << n_power)] = result_value;
        }
        else
        {
            output1[index + (block_y << n_power)] = result_value;
        }
    }
...
#pragma unroll 2
                                      ...
                                      int l, int d_tilda, int d, int r_prime)
    {
        ...
        int I_location = I_location_[block_y];
        int location_out =
            idx + (I_location << n_power) + (((l + 1) << n_power) * block_z);
...
#pragma unroll
        for (int i = 0; i < r_prime; i++)
        {
            ...
            double div = static_cast<double>(partial[i]);
            double mod = static_cast<double>(B_base[i].value);
            r += (div / mod);
        }
        // r = roundf(r);
        r = round(r);
...
#pragma unroll
            for (int j = 0; j < r_prime; j++)
            {
                ...
                // Data partial_ = partial[j] % modulus[i + I_location].value;
                Data mult = VALUE_GPU::mult(
                    partial_,
                ...
                // temp = VALUE_GPU::reduce(temp, modulus[i + I_location]); //
                // new
            }
...
#pragma unroll
                                    ...
                                    Modulus* modulus, int n_power)
    {
        ...
        output[location] =
            VALUE_GPU::add(input1[location], input2[location], modulus[idy]);
    }
