...
namespace heongpu
{
    ...
    __global__ void enc_div_lastq_kernel(Data64* pk, Data64* e, Data64* plain,
                                         Data64* ct, Modulus64* modulus, Data64 half,
                                         ...
                                         int decomp_mod_count)
    {
        ...
        last_pk = OPERATOR_GPU_64::add(last_pk, last_e, modulus[decomp_mod_count]);

        last_pk = OPERATOR_GPU_64::add(last_pk, half, modulus[decomp_mod_count]);

        Data64 zero_ = 0;
        last_pk = OPERATOR_GPU_64::add(last_pk, zero_, modulus[block_y]);

        last_pk = OPERATOR_GPU_64::sub(last_pk, half_mod[block_y], modulus[block_y]);
        ...
        if (block_z == 0)
        {
            ...
            Data64 ct_0 = OPERATOR_GPU_64::mult(message, coeffdiv_plain[block_y],
                                        modulus[block_y]);
        ...
        int Q_prime_size, int Q_size, int P_size)
    {
        ...
        for (int i = 0; i < P_size; i++)
        {
            ...
            Data64 last_e_ = e[idx + ((Q_size + i) << n_power) +
                             ((Q_prime_size << n_power) * block_z)];
            last_pk[i] = OPERATOR_GPU_64::add(last_pk_, last_e_, modulus[Q_size + i]);
        }
        ...
        for (int i = 0; i < P_size; i++)
        {
            ...
            last_pk_add_half_ = OPERATOR_GPU_64::add(last_pk_add_half_, half[i],
            ...
            for (int j = 0; j < (P_size - 1 - i); j++)
            {
                ...
                temp1 = OPERATOR_GPU_64::sub(last_pk[j], temp1, modulus[Q_size + j]);
            ...
            Data64 temp1 =
                OPERATOR_GPU_64::add(last_pk_add_half_, zero_, modulus[block_y]);
                                              ...
                                              int P_size)
    {
        ...
        for (int i = 0; i < P_size; i++)
        {
            ...
            {
                Data64 temp1 =
                    OPERATOR_GPU_64::reduce(last_pk_add_half_, modulus[Q_size + j]);
            ...
            // modulus[block_y]);
            Data64 temp1 =
                OPERATOR_GPU_64::reduce_forced(last_pk_add_half_, modulus[block_y]);
