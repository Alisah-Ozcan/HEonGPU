        ...
        input_storage_manager(
            ...
            [&](Secretkey<Scheme::CKKS>& sk_)
            {
                output_storage_manager(
                    ...
                    [&](Relinkey<Scheme::CKKS>& rk_)
                    {
                        int max_depth = context_->Q_size - 1;
                        ...
                        {
                            int d = context_->d_leveled->operator[](i);
                            int d_tilda =
                                context_->d_tilda_leveled->operator[](i);
                            int r_prime = context_->r_prime_leveled;

                            int counter = context_->Q_prime_size;
                            int location = 0;
                            for (int j = 0; j < i; j++)
                            {
                                location += counter;
                            ...
                            int depth_mod_size = context_->Q_prime_size - i;

                            RandomNumberGenerator::instance()
                                .modular_uniform_random_number_generation(
                                    a_poly, context_->modulus_->data(),
                                    context_->n_power, depth_mod_size,
                                    context_->prime_location_leveled->data() +
                                        location,
                                    d, options.stream_);
                            ...
                            relinkey_gen_II_leveled_kernel<<<
                                dim3((context_->n >> 8), depth_mod_size, 1),
                                256, 0, options.stream_>>>(
                                temp_calculation.data(), sk.data(), error_poly,
                                a_poly, context_->modulus_->data(),
                                context_->factor_->data(),
                                context_->Sk_pair_leveled->operator[](i).data(),
                                context_->n_power, depth_mod_size, d,
                                context_->Q_size, context_->P_size,
                                context_->prime_location_leveled->data() +
                                    location);
                            HEONGPU_CUDA_CHECK(cudaGetLastError());
                            ...
                            relinkey_DtoB_leveled_kernel<<<
                                ...
                                context_->base_change_matrix_D_to_B_leveled
                                    ...
                                    operator[](i)
                                    .data(),
                                context_->Mi_inv_D_to_B_leveled->operator[](i)
                                    .data(),
                                context_->prod_D_to_B_leveled->operator[](i)
                                    .data(),
                                context_->I_j_leveled->operator[](i).data(),
                                context_->I_location_leveled->operator[](i)
                                    .data(),
                                context_->n_power, depth_mod_size, d_tilda, d,
                                r_prime,
                                context_->prime_location_leveled->data() +
                                    location);
                            HEONGPU_CUDA_CHECK(cudaGetLastError());
