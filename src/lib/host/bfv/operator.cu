...
namespace heongpu
{
        ...
        const cudaStream_t stream)
    {
        ...
        const int d_tilda = context_->d_tilda;
        const int r_prime = context_->r_prime;

        DeviceVector<Data64> temp_relin_new(
            (context_->n * d * r_prime) +
                (2 * context_->n * d_tilda * r_prime) +
                (2 * context_->n * context_->Q_prime_size),
            stream);
        Data64* temp1_relin_new = temp_relin_new.data();
        Data64* temp2_relin_new = temp1_relin_new + (context_->n * d * r_prime);
        Data64* temp3_relin_new =
            temp2_relin_new + (2 * context_->n * d_tilda * r_prime);

        base_conversion_DtoB_relin_kernel<<<dim3((context_->n >> 8), d, 1), 256,
                                            0, stream>>>(
            input1.data() + (context_->Q_size << (context_->n_power + 1)),
            temp1_relin_new, context_->modulus_->data(),
            context_->B_prime_->data(),
            context_->base_change_matrix_D_to_B_->data(),
            context_->Mi_inv_D_to_B_->data(), context_->prod_D_to_B_->data(),
            context_->I_j_->data(), context_->I_location_->data(),
            context_->n_power, context_->Q_size, d_tilda, d, r_prime);
        HEONGPU_CUDA_CHECK(cudaGetLastError());
        ...
        gpuntt::GPU_NTT_Inplace(
            temp1_relin_new, context_->B_prime_ntt_tables_->data(),
            context_->B_prime_->data(), cfg_ntt, d * r_prime, r_prime);

        // TODO: make it efficient
        if (relin_key.storage_type_ == storage_type::DEVICE)
        {
            multiply_accumulate_extended_kernel<<<
                dim3((context_->n >> 8), r_prime, d_tilda), 256, 0, stream>>>(
                temp1_relin_new, relin_key.data(), temp2_relin_new,
                context_->B_prime_->data(), context_->n_power, d_tilda, d,
                r_prime);
            HEONGPU_CUDA_CHECK(cudaGetLastError());
        }
        else
        ...
        gpuntt::ntt_rns_configuration<Data64> cfg_intt = {
            ...
            .reduction_poly = gpuntt::ReductionPolynomial::X_N_plus,
            .zero_padding = false,
            .mod_inverse = context_->B_prime_n_inverse_->data(),
            .stream = stream};

        gpuntt::GPU_INTT_Inplace(temp2_relin_new,
                                 context_->B_prime_intt_tables_->data(),
                                 context_->B_prime_->data(), cfg_intt,
                                 2 * r_prime * d_tilda, r_prime);

        base_conversion_BtoD_relin_kernel<<<
            dim3((context_->n >> 8), d_tilda, 2), 256, 0, stream>>>(
            temp2_relin_new, temp3_relin_new, context_->modulus_->data(),
            context_->B_prime_->data(),
            context_->base_change_matrix_B_to_D_->data(),
            context_->Mi_inv_B_to_D_->data(), context_->prod_B_to_D_->data(),
            context_->I_j_->data(), context_->I_location_->data(),
            context_->n_power, context_->Q_prime_size, d_tilda, d, r_prime);
        HEONGPU_CUDA_CHECK(cudaGetLastError());

        divide_round_lastq_extended_kernel<<<
            dim3((context_->n >> 8), context_->Q_size, 2), 256, 0, stream>>>(
            temp3_relin_new, input1.data(), input1.data(),
            context_->modulus_->data(), context_->half_p_->data(),
            context_->half_mod_->data(), context_->last_q_modinv_->data(),
